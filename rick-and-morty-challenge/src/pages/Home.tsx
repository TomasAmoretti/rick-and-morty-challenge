...
export default function Home() { 
  ...
  const [character1Selected, setCharacter1Selected] = useState<number | null>(null); 
  const [character2Selected, setCharacter2Selected] = useState<number | null>(null); 
        ...
        <div className="grid"> 
          {data1?.results?.map((c) => ( 
            <CharacterCard 
              ...
              onClick={() => setCharacter1Selected(c.id)} 
            /> 
          ))} 
        </div> 
      </section> 
        ...
        <div className="grid"> 
          {data2?.results?.map((c) => ( 
            <CharacterCard 
              ...
              onClick={() => setCharacter2Selected(c.id)} 
            /> 
          ))} 
        </div> 
      </section> 
